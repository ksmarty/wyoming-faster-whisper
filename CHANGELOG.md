...
- Add a `gpu` Docker tag (`Dockerfile.gpu`) that runs faster-whisper, transformers, FunASR, onnx-asr, and qwen3-asr on an NVIDIA GPU: CUDA torch plus `onnxruntime-gpu` on a CUDA 12.8 / cuDNN 9 base. `--device cuda` is the default there; `docker run --gpus all` and the NVIDIA Container Toolkit are required
